...
이 함수는 보안 이벤트 실시간 스트림(WebSocket Events Stream) 의 연결 수명주기 관리에 필수적인 구성 요소입니다.
...
- TTL 기반 정리 기능을 따로 포함하지 않으며, connect 함수에서 설정된 TTL 의존
### TODO
- 비정상 종료 감지를 위한 헬스체크 또는 heartbeat 기반 정리 기능 추가
- connectionId 외 확장 정보(account, region, sourceIp 등)의 자동 정리 기능 확대
- WebSocket 연결 상태 모니터링 특정 메트릭 강화
